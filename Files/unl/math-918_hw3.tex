...
\item Conclude that the following are equivalent: 
    ...
    \item $\bigcap_{e> 0}\mf m^{[p^e]}:(I^{[p^e]}:I)$ is contained in the union of the minimal primes of $I$.\footnote{Fun fact: By prime avoidance, you could even restate this a fourth way and require that the intersection by contained in a single minimal prime!}
    \end{enumerate}
\end{enumerate}
...
\begin{enumerate}[itemsep=1em]
\item (Lucas's Theorem) Write $n=\sum_{i=0}^d n_i p^i$ and $m = \sum_{i=0}^d m_i p^i$ where $0\le n_i,m_i<p$ for all $i$.\footnote{Hint: Both parts (a) \& (b) are still true if you allow the top coefficient to be even larger, i.e., if we relax ONLY the last inequality to just be $n_d,m_d,m_{d,i}\ge 0$. You may actually find it easier to prove this more general version!} Then
\[
...
\item Show that $R$ is strongly $F$-regular if $n < d$ and $p \gg 0$. If you like, you may additionally assume that $p-1$ is divisible by $d$. \footnote{Hint: Use ``test element'' criterion (Thm 3.11 of [MP]; done in-class on Tuesday 02/24); an algebraic geometry criterion for non-singularity;  and Lucas's theorem.}
\end{enumerate}
